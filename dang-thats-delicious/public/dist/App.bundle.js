...
function updateActive(resultsElement, direction) {
  ...
  var currentActive = resultsElement.querySelector('.' + activeClass);

  var newActive = void 0;

  if (currentActive) {
    newActive = direction === 'up' ? currentActive.previousElementSibling : currentActive.nextElementSibling;
  }

  if (!newActive) {
    newActive = direction === 'up' ? resultsElement.lastElementChild : resultsElement.firstElementChild;
  }
...
function navigateToActive(resultsElement) {
  var currentActive = resultsElement.querySelector('.search__result--active');
...
(0, _bling.$$)('form.heart .heart__button').on('animationend', function (event) {
  return event.target.classList.remove('heart__button--float');
});
